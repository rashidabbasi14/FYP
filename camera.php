...
<html>
	<script>
		...
		{
			$("#SubDiv").load("CamerasPage.php?CameraID="+clicked_id); 
		}
	</script>
	<body>
		<div id="SubDiv">
			...
			<?php
				...
				$sql="select * from surveillance_camera;";
				...
				if($result->num_rows > 0)
				{
					while($row=$result->fetch_assoc())
					{
						$camid=$row["Camera_ID"];
						$cam_name=$row["Name"];
						$cam_Address=$row["Location"];
						$cam_lng=$row["lng"];
						$cam_lat=$row["lat"];
						echo "
						<a onClick='reply_click(this.id)' id='$camid'>
							<div class='media' style='width:100%;'>
								<img src='images/camera.jpg' height='100px' width='150px' style='margin-right:20px;border-radius: 50%;'>
								<div style='position:relative;'>
									<div class='price'><p style='display:inline;color:white;text-transform:capitalize;'>$cam_name</p><small>Latitude: $cam_lat<br>Longitude: $cam_lng</small></div>
									<div class='stats'>
										<span><i class='fa fa-arrows-alt'></i>$cam_Address</span>
									</div>
						...
						<div style='width:10px;top:40%; right:20px;'><input value='$camid' id='$camid' type='checkbox' style='display:none;'></div>
						";
	...
	<script>
		...
		{ 
			...
			var url = "PhpDB/delCameraFromDB.php";
