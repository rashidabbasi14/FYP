...
<script>
		...
		{
			location.hash="#CulPage";
			$("#SubDiv").load("CulpritsPage.php?Culprit="+clicked_id); 
		}
</script>
	...
	<div id="SubDiv">
		...
		<?php
			...
			if($result->num_rows > 0)
			{
				...
				{
					$user=$row["Name"];
					$userid=$row["Culprit_ID"];
					$status=$row["Description"];
					$sql="select COUNT(*) as Count from tracking where Culprit_ID = $userid";
					...
					$sql="select path from culprit_pictures where Culprit_ID=$userid;";
					$result1=$con->query($sql);
					if($result1->num_rows > 0)
					{
						$row1=$result1->fetch_assoc();
				...
				echo "
					<a onClick='reply_click(this.id)' id='$userid'>
						<div class='media' style='padding:10px;'>
							<img src='".$path."' height='100px' width='150px' style='margin-right:20px;border-radius: 50%;'>
							<div style='position:relative;'>
								<div class='price'><p style='display:inline;color:white;text-transform:capitalize;'>".$user."</p><small style='padding-top:5px;'>$status</small></div>
								<div class='stats'>
									<span><i class='fa fa-arrows-alt'></i>Found $count times.</span>
								</div>
								</div>
								<div style='position:absolute; top:40%; right:10px;'><input value='".$user."' id='cbox".$user."' type='checkbox' style='display:none;'></div>
						</div>
					</a>
				";
				}
